...
import { getBiometricAccessById, updateBiometricAccessById } from 'apiSdk/biometric-accesses';
...
function BiometricAccessEditPage() {
  ...
  const handleSubmit = async (values: BiometricAccessInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBiometricAccessById(id, values);
      mutate(updated);
      resetForm();
