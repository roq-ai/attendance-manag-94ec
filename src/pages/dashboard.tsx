...
import { getBiometricAccesses } from 'apiSdk/biometric-accesses';
...
function DashboardPage() {
  ...
  const biometricAccessStat = useSWR<PaginatedInterface<BiometricAccessInterface>>(
    () => `/biometric-accesses`,
    () => getBiometricAccesses({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Biometric Accesses',
      result: biometricAccessStat?.data?.totalCount,
      isLoading: biometricAccessStat.isLoading,
    },
  ];
